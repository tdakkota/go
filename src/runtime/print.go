...
|совет от мимоны                                               |
|                                                              |
|Go это почти что динамически типизированный язык, потому      |
|что система типов слабееькаяи очень часто используются        |
|interface{} + type-switch которые по сути и есть динамическая |
|типизация                                                     |
+--------------------------------------------------------------+` + "\n"

func printwtf() {
	printstring(theTruth)
}
